...
export default function LeaveReviewPage() {
  ...
  const handleSubmit = async () => {
    ...
    if (!job?.assignedWorkerId) {
      toast.error('No worker assigned to this job.')
      return
    }
    if (rating === 0) {
    ...
    try {
      const res = await fetch('/api/reviews', {
        ...
        body: JSON.stringify({
          jobId,
          reviewerId: user.uid,
          revieweeId: job.assignedWorkerId,
          workerId: job.assignedWorkerId,
          homeownerId: user.uid,
          rating,
          comment: comment.trim(),
          tags: selectedTags,
          reviewerName: profile.displayName ?? user.displayName ?? 'Homeowner',
      ...
      // Redirect to job page with reviewed flag so toast shows
      router.push(`/jobs/${jobId}?reviewed=true`)
    } catch {
  ...
  if (!authLoading && !user) {
    router.push(`/auth/login?redirect=/jobs/${jobId}/review`)
    return null
  }

  // Only the homeowner/employer for this job can review
  const isJobEmployer = user?.uid === job?.employerId
  if (job && !isJobEmployer) {
    return (
      <div className="flex flex-col min-h-screen">
        <Navbar />
        <main className="flex-1">
          <div className="max-w-2xl mx-auto px-4 py-16 text-center">
            ...
            <p className="text-gray-500 text-sm">Only the homeowner who posted this job can leave a review.</p>
        ...
        <div className="max-w-2xl mx-auto px-4 sm:px-6 py-8">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 sm:p-8">
            <div className="flex items-center gap-2 mb-6">
              ...
              <h1 className="text-xl font-bold text-gray-900 dark:text-white">Leave a Review</h1>
            </div>
              ...
              <div className="mb-6 p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                ...
                {worker && (
                  <p className="text-sm text-gray-500 mt-1">
                    Tradie: <span className="font-medium text-gray-700 dark:text-gray-300">{worker.displayName}</span>
                  </p>
                )}
              </div>
            )}
              ...
              <div className="flex items-center gap-3 p-4 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg">
                ...
                <div>
                  ...
                  <p className="text-sm text-green-700 dark:text-green-400 mt-0.5">
                    You have already reviewed this job. Thank you for your feedback!
                  </p>
                </div>
              </div>
            ) : (
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="Describe your experience — was the work completed to a high standard? Was the tradie punctual and professional?"
                    maxLength={500}
