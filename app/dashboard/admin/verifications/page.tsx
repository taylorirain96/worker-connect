...
export default function AdminVerificationsPage() {
  ...
  async function loadRecords() {
    setLoading(true)
    try {
      const res = await fetch('/api/worker-verifications')
      if (res.ok) {
  ...
  async function handleAction(uid: string, action: 'approve' | 'reject') {
    ...
    try {
      const res = await fetch('/api/worker-verifications/review', {
        method: 'POST',
    ...
    <div className="p-6 space-y-6">
      ...
      {loading ? (
        ...
        <div className="bg-slate-800 rounded-xl p-12 text-center text-slate-400">
          No {filter === 'all' ? '' : filter} ID verifications found.
        </div>
      ) : (
                    ...
                    <div className="flex items-center gap-2 mt-0.5">
                      ...
                      {record.documentType && (
                        <span className="text-xs text-slate-500 bg-slate-700 px-1.5 py-0.5 rounded flex-shrink-0">
                          {DOCUMENT_LABELS[record.documentType] ?? record.documentType}
                        </span>
                      )}
                    </div>
                  </div>
                ...
                {isExpanded && (
                  <div className="px-4 pb-4 space-y-4 border-t border-slate-700">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4">
                      <ImagePreview url={record.frontImageUrl} label="Front of ID" />
                      <ImagePreview url={record.backImageUrl} label="Back of ID" />
                    </div>
