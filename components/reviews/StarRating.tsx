'use client'

import { Star } from 'lucide-react'
import { cn } from '@/lib/utils'

interface StarRatingProps {
  /** Current rating value (0–5, supports decimals) */
  value: number
  max?: number
  size?: 'sm' | 'md' | 'lg'
  /** If true, the component becomes an interactive star picker */
  interactive?: boolean
  onRate?: (rating: number) => void
  className?: string
}

const SIZE_MAP = {
  sm: 'h-3.5 w-3.5',
  md: 'h-5 w-5',
  lg: 'h-7 w-7',
}

/**
 * Reusable star rating component.
 * Supports both read-only display and interactive modes.
 * Accepts a `value` prop (alias for the `rating` prop in RatingStars).
 */
export default function StarRating({
  value,
  max = 5,
  size = 'md',
  interactive = false,
  onRate,
  className,
}: StarRatingProps) {
  const starSize = SIZE_MAP[size]

  return (
    <div
      className={cn('flex items-center gap-0.5', className)}
      role={interactive ? 'radiogroup' : undefined}
      aria-label={`Rating: ${value} out of ${max}`}
    >
      {Array.from({ length: max }, (_, i) => {
        const starValue = i + 1
        const filled = starValue <= Math.round(value)
        const partial = !filled && starValue - 0.5 <= value

        return (
          <button
            key={i}
            type="button"
            disabled={!interactive}
            onClick={() => interactive && onRate?.(starValue)}
            className={cn(
              'focus:outline-none transition-transform',
              interactive && 'cursor-pointer hover:scale-110 focus:scale-110',
              !interactive && 'cursor-default pointer-events-none',
            )}
            aria-label={interactive ? `Rate ${starValue} star${starValue > 1 ? 's' : ''}` : undefined}
          >
            <Star
              className={cn(
                starSize,
                filled || partial
                  ? 'fill-yellow-400 text-yellow-400'
                  : 'fill-gray-200 text-gray-300 dark:fill-gray-700 dark:text-gray-600',
              )}
            />
          </button>
        )
      })}
    </div>
  )
}
